...
export const downloadSingleFile = async (
	...
	try {
		...
		if (!(blob instanceof Blob)) {
			...
			if (!file.file_share_state?.public_file.cid_original_decrypted && file.encryption_status === EncryptionStatus.Encrypted) {
				const originalCid = file.cid_original_encrypted;
				binaryData = await blobToArrayBuffer(binaryData).catch(() => {
			...
			if (file.file_share_state && file.file_share_state.id !== 0) {
				...
				if (originalCid != "") {
					binaryData = await blobToArrayBuffer(binaryData).catch(() => {
						console.log("Error downloading shared file");
					});
					binaryData = await decryptFileBuffer(
						binaryData,
						originalCid,
