...
const getAesKey = async (
  ...
  salt?: Uint8Array,
  iv?: Uint8Array
): Promise<{ aesKey: CryptoKey; salt: Uint8Array; iv: Uint8Array }> => {
...
const getResultBytes = (
  cipherBytesArray: Uint8Array,
  salt: Uint8Array,
  iv: Uint8Array
): Uint8Array => {
...
const decryptContentUtil = async (
...
export const decryptMetadata = async (
  encryptedFilenameBase64Url: string,
  encryptedFiletypeBase64Url: string,
  cidOriginalEncrypted: string,
  personalSignature: string | undefined
): Promise<
  | {
      decryptedFilename: string;
      decryptedFiletype: string;
      decryptedCidOriginal: string;
    }
  | undefined
> => {
  if (!personalSignature) {
  ...
  const decryptValue = async (cipherBytes: Uint8Array) => {
    const decryptedValueArrayBuffer = await decryptContent(
      cipherBytes,
      personalSignature
    );
...
export const encryptFileBuffer = async (fileArrayBuffer: ArrayBuffer) => {
  try {
    ...
    const encryptionTimeParsed =
      ...
      encryptionTime.toFixed(2).toString() +
      " " +
      encryptionSuffix;
    ...
    return {
      ...
      encryptedFileBuffer: resultBytes,
