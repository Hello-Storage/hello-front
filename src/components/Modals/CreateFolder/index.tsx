...
export default function CreateFolderModal() {
  ...
  const getRoot = () => (window.location.pathname.includes("/folder") ? window.location.pathname.split("/")[2] : "/");
  ...
  const handleCreateNewFolder = async () => {
    ...
    if (encryptionEnabled) {
      ...
      const encryptedTitleBuffer = await encryptBuffer(new TextEncoder().encode(title), personalSignature);
    ...
    Api.post("/folder/create", { root: root, title: titleFinal, status: status })
      .then((resp) => {
