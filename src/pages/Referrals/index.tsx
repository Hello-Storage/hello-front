import { HiClipboard, HiOutlinePaperAirplane, HiOutlineUsers } from "react-icons/hi";
...
const Referrals = () => {
    ...
    useEffect(() => {
        Api.get(`/referrals/${walletAddress}`).then((res) => {
    ...
    const onSubmit = (values: any, { setSubmitting }: any) => {
        setTimeout(() => {
            ...
            window.open(`mailto:${values.email}?subject=Join me on Hello&body=Hi,%0D%0A%0D%0AI would like to invite you to join Hello Storage. It is a decentralized storage infrastructure across thousands of nodes around the world.%0D%0A%0D%0AYou can get 10GB of free storage for each referred user.%0D%0A%0D%0AHere is my referral link:%0D%0A${referralLink}`);
            setSubmitting(false);
        }, 400);
    };
        ...
        <div className="flex flex-col h-full relative">
            ...
            <div className="h-full p-8 flex flex-col justify-center items-center border rounded-xl">
                <div className="mb-4 flex items-center justify-center text-center w-full space-x-2">
                    ...
                    <h1 className="text-3xl select-none text-center text-violet-700 hover:text-violet-600">Get +10GB free for each referred user!</h1>
                </div>
                <div className="flex-grow flex flex-col justify-center items-center">
                    ...
                    >
                        {({ isSubmitting }) => (
                            <div className="mb-6 pt-6 relative rounded-md shadow-sm w-3/4 max-w-md">
                                <ErrorMessage name="email" component="div" className="absolute top-0 left-0 w-full text-red-500" />
                    ...
                    <div className="flex justify-between border border-gray-300 rounded-md p-4">
                        <div>
                            <p className="px-4 text-lg font-medium">{totalUsers}/{maxUsers} referred users.</p>
                        </div>
                        <div>
                            <p className="px-4 text-lg font-medium">{totalUsers * 10}GB/{maxUsers * 10}GB storage gained.</p>
                        </div>
                    </div>
                    <div>
                        <p className="pt-10 w-full text-lg text-left">You got {totalUsers * 10}GB/{maxUsers * 10}GB from {totalUsers} invited users</p>
                        <div className="grid grid-cols-9 gap-2">
                            {Array.from({ length: maxUsers }).map((_, index) => (
                                <div
                                    key={index}
                                    className={`h-8 w-20 p-2 rounded ${index < totalUsers ? 'bg-green-500' : 'bg-red-500'} hover:bg-blue-500 fex items-center`}
                                    title={index < totalUsers ? `Invited user wallet address: ${referredAddresses[index]}` : ''}
                                ><p className="text-white leading-none select-none">{index < totalUsers ? `+10GB` : ''}</p></div>
                            ))}
                        </div>
                    </div>
                </div>
